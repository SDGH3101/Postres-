...
class ProductoApiController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $p = Producto::create($request->only('descripcion', 'precio', 'stock', 'fecha_caducidad', 'stock_minimo'));
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$p) return response()->json(['error' => 'No encontrado'], 404);
        $request->validate([
            'descripcion'      => 'sometimes|required|max:200',
            'precio'           => 'sometimes|required|numeric|min:0',
            'stock'            => 'sometimes|required|integer|min:0',
            'fecha_caducidad'  => 'nullable|date',
            'stock_minimo'     => 'nullable|integer|min:0',
        ]);
        $p->update($request->only('descripcion', 'precio', 'stock', 'fecha_caducidad', 'stock_minimo'));
