...
{
    // Mapea el periodo elegido (diario/semanal/mensual/anual) a expresiones SQL
    private function periodoSql(string $periodo): array
    {
        return match ($periodo) {
            'diario'  => [
                'grupo'  => "DATE_FORMAT(fecha,'%d-%m-%Y')",
                'orden'  => "fecha",
                'limite' => "fecha >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)",
            ],
            'semanal' => [
                'grupo'  => "CONCAT('Sem ', WEEK(fecha,3), ' - ', YEAR(fecha))",
                'orden'  => "YEARWEEK(fecha,3)",
                'limite' => "fecha >= DATE_SUB(CURDATE(), INTERVAL 16 WEEK)",
            ],
            'anual'   => [
                'grupo'  => "YEAR(fecha)",
                'orden'  => "YEAR(fecha)",
            ...
            default   => [ // mensual
                'grupo'  => "DATE_FORMAT(fecha,'%m-%Y')",
                'orden'  => "DATE_FORMAT(fecha,'%Y-%m')",
                'limite' => "fecha >= DATE_SUB(CURDATE(), INTERVAL 12 MONTH)",
            ],
        };
    }
    ...
    public function index(Request $request)
    {
        ...
        ['grupo' => $g, 'orden' => $o, 'limite' => $lim] = $this->periodoSql($periodo);
        ...
        $masVendidos = DB::select("
            SELECT p.descripcion, p.precio, p.stock,
                   COUNT(d.id_detalle_venta) AS veces, SUM(d.subtotal) AS ingresos
            FROM producto p
        ...
        $balance = DB::select("
            ...
            FROM (
                SELECT $g AS mes, SUM(total) AS total_ventas
                FROM venta WHERE $lim GROUP BY $g
            ) ventas
            LEFT JOIN (
                SELECT $g AS mes, SUM(monto) AS total_gastos
                FROM gasto WHERE $lim GROUP BY $g
            ) gastos ON ventas.mes = gastos.mes
            ORDER BY ventas.mes
        ");

        return view('reportes.index', compact('ventasMes','gastosCat','masVendidos','balance','periodo'));
    }
    ...
    public function ventasPdf()
    {
        $ventas = DB::select("
            ...
            GROUP BY v.id_venta, v.fecha, u.nombre, v.total
            ORDER BY v.fecha
        ");
