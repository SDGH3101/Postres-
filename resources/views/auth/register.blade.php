...
<div class="auth-wrap">
  ...
  <div class="auth-card">
    <div class="auth-tabs">
      ...
      <a href="{{ route('register') }}" class="auth-tab on">Crear cuenta</a>
    </div>
    ...
    <form method="POST" action="{{ route('register.post') }}">
      ...
      <div class="fg" style="margin-bottom:14px">
        ...
        <input type="email" name="correo" value="{{ old('correo') }}" placeholder="tu@correo.com" class="{{ $errors->has('correo') ? 'is-invalid':'' }}">
        @error('correo')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="fg" style="margin-bottom:14px">
        <label>Fecha de nacimiento</label>
        <input type="date" name="fecha_nacimiento" value="{{ old('fecha_nacimiento') }}" max="{{ now()->subYears(16)->toDateString() }}" class="{{ $errors->has('fecha_nacimiento') ? 'is-invalid':'' }}">
        @error('fecha_nacimiento')<div class="invalid-feedback">{{ $message }}</div>@enderror
      </div>
      <div class="fg" style="margin-bottom:14px">
        <label>Contraseña (mín. 6 caracteres)</label>
        <input type="password" name="pass" placeholder="••••••••" class="{{ $errors->has('pass') ? 'is-invalid':'' }}">
