...
<div class="fcard">
  ...
  <form method="POST" action="{{ route('usuarios.update', $usuario->id_usuario) }}">
    ...
    <div class="fgrid">
      ...
      <div class="fg"><label>Fecha de nacimiento</label>
        <input type="date" name="fecha_nacimiento" value="{{ old('fecha_nacimiento', $usuario->fecha_nacimiento) }}"
               ...
               min="{{ now()->subYears(99)->toDateString() }}" class="{{ $errors->has('fecha_nacimiento')?'is-invalid':'' }}">
        @error('fecha_nacimiento')<div class="invalid-feedback">{{ $message }}</div>@enderror</div>
